...
export default class GuideCommand extends Command {
	...
	public async exec(msg: Message): Promise<Message | Message[]> {
		const prefix = msg.guild ? this.client.settings!.guild.get(msg.guild.id)!.prefix : 'g';
		const embed = this.client.util.embed()
			...
			`)
			.addField('Starting a Giveaway', stripIndents`
		...
		if (this.client.user!.id === process.env.ID) {
			embed.addField('Other', stripIndents`
			Get stuck? Come visit our [Support Server](https://discord.sycer.dev/) to get help!

			Want to invite **${this.client.user!.username}** to your server? You can invite it with [this](${await this.client.generateInvite(346176)}) link!
		`);
		}
