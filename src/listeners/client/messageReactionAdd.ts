...
export default class MessageReactionAddListener extends Listener {
	...
	public async exec(reaction: MessageReaction, user: User): Promise<void> {
		...
		const embed = this.client.util.embed()
			...
			.setThumbnail('https://cdn.discordapp.com/emojis/358336932343840769.png?v=1')
			.setDescription(stripIndents`
                Drop ended!

                **Winner${doc.winnerCount === 1 ? '' : 's'}**: ${users.map(r => r.toString()).join(', ').substring(0, 1800)}
            `);
		...
		if (msg.channel instanceof TextChannel && msg.channel.permissionsFor(this.client.user!)!.has('SEND_MESSAGES')) {
			await msg.channel.send(`🎉 Congratz, ${users.map(u => u.toString()).join(', ')}! You won the FCFS drop for **${doc.title}**!`);
		}
	}
